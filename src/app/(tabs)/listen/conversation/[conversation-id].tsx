...
export default function ConversationScreen() {
  ...
  const { conversation, messages, addMessage } = useChatConversation(conversationId);
    ...
    <Screen className="bg-lightGrey">
      ...
      <KeyboardAvoidingView
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <MessageBubble
                ...
                onSpeak={() => speech.speak(item.body, item.language as 'en-US' | 'ar-EG', item.id)}
              />
            )}
          />
        ) : (
        ...
        <ConversationInput
          ...
          onAddTextToSpeech={(text) => addMessage(text, 'text-to-speech', language)}
          onAddSpeechToText={(text) => addMessage(text, 'speech-to-text', language)}
        />
      </KeyboardAvoidingView>
    </Screen>
  );
